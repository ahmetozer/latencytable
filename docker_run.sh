#!/bin/bash
echo "Latency Table Website Container"
if [ "$webserver" == "yes" ]
then
    ...
    then
        ...
        ssl_certificate_key /etc/ssl/private/nginx-selfsigned.key;" > /etc/nginx/project_cert_dir.conf
    else
        echo "ssl_certificate     /etc/ssl/certs/project.crt;
        ssl_certificate_key /etc/ssl/private/project.key;" > /etc/nginx/project_cert_dir.conf
    fi
fi
